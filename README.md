...
A multi-service deception system deployed to capture and analyse attacker behaviour. See the [honeypot repository](https://github.com/adesinaijo/honeypot-alert-system) for full details.
...
|-------|----------|-------------|
| `security-admins` | EC2FullAccess, VPCFullAccess, CloudWatchFullAccess | Full infrastructure |
| `developers` | EC2ReadOnly, CloudWatchReadOnly, VPCReadOnly | Read-only infrastructure |
| `hr-team` | IAMReadOnly | User and role visibility only |
...
2. Launch EC2 instance, configure security group
3. Install and configure UFW
4. Install Nginx, obtain SSL certificate via Certbot
5. Install WireGuard, generate server and client keys
6. Deploy Honeypot Alert System
7. Configure MongoDB
8. Set up systemd services for auto-restart
9. Configure IAM groups and users
...
| Tunnel | WireGuard | Encrypted VPN for direct access |
| Identity | AWS IAM | Role-based access control |
...
## Related Projects
- [Honeypot Alert System](https://github.com/adesinaijo/honeypot-alert-system)
- [Global Vulnerability Aggregator](https://github.com/adesinaijo/vuln-aggregator)
- [Security Policy Generator](https://github.com/adesinaijo/securityPolicyGenerator)
...
[GitHub](https://github.com/adesinaijo)
